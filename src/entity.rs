...
impl<T: 'static> Storage<T> {
    ...
    pub fn get(&self, entity: Entity) -> ObjRef<T> {
        self.obj_inner(entity).get()
    }

    pub fn get_mut(&self, entity: Entity) -> ObjMut<T> {
...
pub struct Entity(NonZeroU64, PhantomData<*const ()>);
...
impl Entity {
    ...
    pub fn get<T: 'static>(self) -> ObjRef<T> {
        storage::<T>().get(self)
    }

    pub fn get_mut<T: 'static>(self) -> ObjMut<T> {
    ...
    ) -> Option<ObjMut<T, Nothing<'_>>> {
        storage::<T>().try_get_mut(self, loaner)
    }

    pub fn get_on_loan<T: 'static>(self, loaner: &ImmutableBorrow<T>) -> ObjRef<T, Nothing<'_>> {
